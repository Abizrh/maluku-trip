...
import { Booking } from "./wisatawanStore";
...
const mockSchedules: TourSchedule[] = [
  {
    ...
    isAvailable: true,
    description:
      "Eksplorasi keindahan pantai kuta dengan pemandu berpengalaman",
  },
  {
    id: "2",
    destinationId: "102",
...
export const usePemanduStore = create<PemanduState>()((set, get) => ({
  ...
  fetchOrderGuides: async () => {
    ...
    try {
      const resp = await apiClient.get("/guide/order");
      ...
      console.error("Fetch order guide error:", error);
      set({
        error: "Gagal mengambil data order guide",
        isLoading: false,
      });
      return [];
    }
  ...
  updateOrderGuide: async (orderGuideId, orderGuideData) => {
    set({ isLoading: true, error: null });
    try {
      const resp = await apiClient.patch(
        `/guide/order/${orderGuideId}/${orderGuideData}`,
      );
      const { data } = resp.data;
      return data;
    } catch (error) {
  ...
  fetchGuides: async () => {
    set({ isLoading: true, error: null });
    try {
      const resp = await apiClient.get("/guide");
      const { data } = resp.data;
      set({
        guides: data,
        isLoading: false,
